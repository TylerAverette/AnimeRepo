...
class AuthViewModel: ObservableObject {
    ...
    func RegisterUser(email: String, password: String) {
        Auth.auth().createUser(withEmail: email, password: password) { authResult, error in
            ...
            do {
                self.userInstance.page = "1"
                try self.db.collection("userData").document((authResult?.user.uid)!).setData(from: self.userInstance) {err in
                    if let err = err {
                        print(err)
                    } else {
                        self.animevm?.currentAnime = AnimeModel(mal_id: 00, title: "", synopsis: "", images: ImageList(jpg: Image(image_url: "", small_image_url: "", large_image_url: "")))
                        self.populateUser()
                    }
                }
            } catch {
    ...
    func populateUser() {
        ...
        userDocument.getDocument { document, error in
            ...
            do{
                ...
                self.animevm?.currentIndex = 0
                self.animevm?.page = Int(self.userInstance.page)!
